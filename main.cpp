...
int main()
{
  ...
  OrderCache example4_cache;
  example4_cache.addOrder(Order{"1", "SecId1", "BUY", 100, "User1", "Company1"});
  example4_cache.addOrder(Order{"2", "SecId1", "SELL", 100, "User2", "Company1"});

  std::vector<Order> allOrders = example4_cache.getAllOrders();
  std::cout << "Orders BEFORE cancelling" << std::endl;
  for (auto & e : allOrders){
    std::cout << e.orderId() << " : " << e.user() << std::endl;
  }

  // Cancel order for User2
  example4_cache.cancelOrdersForUser("User2");
  allOrders = example4_cache.getAllOrders();

  // Cancel order for User1
  example4_cache.cancelOrdersForUser("User1");
  allOrders = example4_cache.getAllOrders();
  std::cout << "Orders AFTER cancelling" << std::endl;
  for (auto & e : allOrders){
    std::cout << e.orderId() << " : " << e.user() << std::endl;
  }
